...
for proto in -4 -6; do
    ...
    /opt/sbin/ip -o "$proto" route show dev "$dev" | \
    while read route; do
        if [ "$route" = "${route/metric/}" ]; then
            # shellcheck disable=SC2086
            /opt/sbin/ip "$proto" route del $route dev "$dev"
            # shellcheck disable=SC2086
            /opt/sbin/ip "$proto" route add $route metric "$metric" dev "$dev"
            continue
        fi
        ...
        if [ "$route" != "$route_new" ]; then
            ...
            /opt/sbin/ip "$proto" route del $route dev "$dev"
            # shellcheck disable=SC2086
            /opt/sbin/ip "$proto" route add $route_new dev "$dev"
        fi
    done
done
