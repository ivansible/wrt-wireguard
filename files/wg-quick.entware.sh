...
add_if() {
        local ret
        if [ "${WG_QUICK_PREFER_USERSPACE:-}" = 'true' ] && command -v "${WG_QUICK_USERSPACE_IMPLEMENTATION:-wireguard-go}" >/dev/null ; then
...
resolvconf_iface_prefix() {
        [[ -f /opt/etc/resolvconf/interface-order ]] || return 0
        local iface
        while read -r iface; do
                [[ $iface =~ ^([A-Za-z0-9-]+)\*$ ]] || continue
                echo "${BASH_REMATCH[1]}." && return 0
        done < /opt/etc/resolvconf/interface-order
}
...
save_config() {
        ...
        done < <(resolvconf -l "$(resolvconf_iface_prefix)$INTERFACE" 2>/dev/null || cat "/opt/etc/resolvconf/run/interface/$(resolvconf_iface_prefix)$INTERFACE" 2>/dev/null)
...
Usage: $PROGRAM [ up | down | save | strip ] [ CONFIG_FILE | INTERFACE ]
  ...
  configuration found at /opt/etc/wireguard/INTERFACE.conf. It is to be readable
